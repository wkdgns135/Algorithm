...
int main() {
    ...
    vector<pair<Pos, Pos>> visited;
    visited.push_back({ startRed,startBlue });
    ...
    while (!bfs.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (any_of(visited.begin(), visited.end(), [&](pair<Pos, Pos>& pos) {return pos.first == newRed && pos.second == newBlue; }))continue;
            visited.push_back({ newRed, newBlue });
